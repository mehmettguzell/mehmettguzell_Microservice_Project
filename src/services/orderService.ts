...
export const getAllOrders = () =>
  fetcher<OrderResponse[]>(`${BASE}/all`, { cache: "no-store" });

export const getOrderById = (id: number) =>
  fetcher<OrderResponse>(`${BASE}/${id}`, { cache: "no-store" });
...
export const confirmOrder = (id: number) =>
  fetcher<OrderResponse>(`${BASE}/confirm/${id}`, {
    cache: "no-store",
    method: "PATCH",
  });
