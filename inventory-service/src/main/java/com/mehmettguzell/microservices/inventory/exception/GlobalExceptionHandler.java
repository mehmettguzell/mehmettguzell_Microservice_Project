...
public class GlobalExceptionHandler {
    ...
    private <T> ResponseEntity<ApiResponse<T>> buildErrorResponse(HttpStatus status, String code, String message) {
        ErrorResponse error = buildError(status, code, message);
        ApiResponse<T> response = new ApiResponse<>(false, "Operation failed", (T) error);
    ...
    public ResponseEntity<ApiResponse<ErrorResponse>> handleInventoryNotFound(InventoryNotFoundException ex) {
        return buildErrorResponse(HttpStatus.NOT_FOUND, ex.getMessage(), ex.getMessage());
    }
    ...
    public ResponseEntity<ApiResponse<ErrorResponse>> handleGenericException(Exception ex) {
        return buildErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR, ex.getMessage(), ex.getMessage());
    }
}
